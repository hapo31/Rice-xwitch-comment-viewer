...
`metadata.message_timestamp` は RFC 3339 として解析し、明示された offset を同じ instant の UTC へ正規化する。小数秒は nanosecond 精度まで保持し、Tauri event の `receivedAt` は末尾 `Z` の UTC RFC 3339 とする。timestamp が欠落、空文字、タイムゾーンなし、または不正な場合も chat notification 自体は破棄せず、その WebSocket frame の受信時刻を使って warning log を残す。frontend は bridge で同じ形式を検証してから store へ渡し、Chat view でのみ利用者のローカルタイムゾーンへ変換して表示する。
