...
- 2026-05-23: devcontainer rebuild 後に Codex の認証情報と履歴が消える問題を防ぐため、`rice-codex-home` named volume を `/home/vscode/.codex` にマウントするようにした。`setup.sh` で所有者と `700` 権限を整える。
- 2026-05-23: Codex 状態永続化のために作成する Docker volume 名が分かるよう、`.devcontainer/README.md` に `docker volume create rice-codex-home` を明記した。
- 2026-05-23: devcontainer rebuild 前後で Codex 状態を手動退避/復元できるよう、書き捨ての `.devcontainer/codex-state-transfer.sh` を追加した。バックアップ zip は git 管理外の `.codex-state-backup/codex-state-backup.zip` に置く。
...
- 2026-05-23: アプリ内の日本語が豆腐表示になる問題を調査。devcontainer に日本語フォントが入っておらず、WebKit/WSLg で CJK fallback が成立しない状態だった。Tailwind の `fontFamily.sans` / `fontFamily.mono` に Windows 標準の日本語フォントと Noto CJK 系 fallback を追加し、devcontainer には `fonts-noto-cjk` を追加した。
