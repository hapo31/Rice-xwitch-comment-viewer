...
- [x] Issue #74: `ChatMessage.received_at` を `DateTime<Utc>` に統一し、offset・小数秒を UTC の `receivedAt` として bridge へ渡す。timestamp の欠落・空文字・タイムゾーンなし・不正値は WebSocket 受信時刻へ fallback して警告し、frontend の境界検証、system/mock message、ローカル時刻表示をテストする。
