...
describe("system timeline routing", () => {
  it("routes EventSub and auth transitions but excludes keepalives", () => {
    expect(timelineEventFromTwitchStatus({ status: "reconnecting", message: "Twitch EventSub が切断されました。", occurredAtMs: 1 })).toMatchObject({ source: "twitch-connection", transition: "reconnecting" });
    expect(timelineEventFromTwitchStatus({ status: "authRequired", message: "Twitch 認証が無効です。再ログインしてください。", occurredAtMs: 1 })).toMatchObject({ source: "twitch-auth", transition: "authRequired:Twitch 認証が無効です。再ログインしてください。" });
    expect(timelineEventFromTwitchStatus({ status: "connected", message: "session_keepalive", occurredAtMs: 1 })).toBeUndefined();
  });
